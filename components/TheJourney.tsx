...
import { useRef, useLayoutEffect, useState, useEffect, useCallback, useId } from 'react';
...
export default function TheJourney({
  ...
  const animateToStep = useCallback(
    (newStep: number, direction: 'up' | 'down') => {
      ...
      if (direction === 'down') {
        ...
        const connector = connectors[newStep - 1];
  ...
  // Scoped Observer on sectionRef — touch-action:none is on the container
  useLayoutEffect(() => {
              ...
              {journeySteps.map((step, index) => {
                ...
                return (
                  <div key={index} className="w-full">
                    <div
                      className={`flex w-full justify-center ${isRight ? 'lg:justify-end' : 'lg:justify-start'}`}
                    >
                      <div
                        ref={(el) => {
